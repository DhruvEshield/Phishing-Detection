"""Tests for Google Safe Browsing detector utility."""
...
def mock_settings():
    ...
    with patch("app.detectors.safe_browsing.get_settings") as mock_get:
        settings = MagicMock()
        settings.google_safe_browsing_key = "test_api_key_123"
...
@patch("app.detectors.safe_browsing.httpx.Client")
def test_overlapping_threat_types_deduplicated(mock_client_class, mock_settings):
    """3. Multiple threat items with overlapping threatTypes correctly deduplicate"""
    mock_client = mock_client_class.return_value.__enter__.return_value
    mock_response = MagicMock()
    mock_response.status_code = 200
    mock_response.json.return_value = {
    ...
    assert set(result["threat_types"]) == {"MALWARE", "UNWANTED_SOFTWARE"}
...
def test_missing_api_key_returns_none(mock_client_class, mock_settings):
    ...
    mock_settings.google_safe_browsing_key = None
